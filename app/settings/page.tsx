...
export default function SettingsPage() {
    ...
    useEffect(() => {
        ...
        // Load from DB on mount — fix for settings not persisting
        fetch('/api/settings').then(r => r.json()).then(d => {
            const inst = d.institute;
            if (inst) {
                setName(inst.name || 'Sharada Classes');
                setAddress(inst.address || 'Main Road, Dapoli, Ratnagiri – 415 712');
                setPhone(inst.phone || '+91 98765 43210');
                setEmail(inst.email || 'info@sharadaclasses.in');
            }
            setLoading(false);
        });
    }, []);
    ...
    return (
        <>
            ...
            .sp2-inp { width:100%; background:rgba(255,255,255,.025); border:1px solid rgba(255,255,255,.07); border-radius:12px; padding:12px 16px; font-size:13.5px; color:#e2e8f0; font-family:'Inter',sans-serif; outline:none; transition:all .2s; }
            ...
            .sp2-btn { width:100%; border:none; border-radius:12px; padding:14px; font-size:14px; font-weight:700; font-family:'Inter',sans-serif; cursor:pointer; transition:all .2s; }
            .sp2-btn-default { background:#f97316; color:white; box-shadow:0 4px 16px rgba(249,115,22,.25); }
            .sp2-btn-default:hover:not(:disabled) { background:#ea580c; transform:translateY(-2px); }
            .sp2-btn-saved { background:#16a34a; color:white; }
            ...
            .sp2-loading { text-align:center; padding:40px; color:#334155; font-size:13px; }
        `}</style>
            <div className={`sp2-root ${visible ? 'show' : ''}`}>
                <div style={{ marginBottom: 24 }}>
                    <div className="sp2-title">Settings</div>
                    <div className="sp2-sub">Institute information shown on all report cards</div>
                </div>

                {loading ? (
                    <div className="sp2-card sp2-loading">Loading settings…</div>
                ) : (
                    <div className="sp2-card">
                        ...
                        <div className="sp2-info" style={{ marginTop: 16 }}>
                            ...
                            <div className="sp2-info-text">
                                This information appears on <b>every PDF report card</b> — institute name, address, phone and email are all printed in the report header.
                            </div>
                        </div>
                    </div>
                )}
